  ...
  return (
    <TouchableOpacity style={[styles.card, cardStyles]} onPress={() => {navigation.navigate('JobDetails', {job:data})}}>
      <View style={styles.rowSpaceBetween}>
        <View style={styles.rowCenter}>
          ...
          {cardType === 'featured' ? (
            ...
            <Text style={styles.titleText0}>{data.title}</Text>
      ...
      <View style={styles.bottomContent}>
        <Text style={styles.salaryText}>{`₹ ${data.salary} - ₹ ${data.salary} LPA`}</Text>
        <View style={styles.rowSpaceBetween}>
          <View style={styles.rowCenter}>
            {data.workMode && <View style={styles.tag}><Text style={styles.tagText}>{data.workMode}</Text></View>}
